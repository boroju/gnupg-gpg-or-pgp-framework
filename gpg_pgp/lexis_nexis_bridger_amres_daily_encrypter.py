...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Python program for Lexis-Nexis Bridger AmRes Daily Encrypter by using Gnupg PGP.',
        add_help=True)
    ...
    parser.add_argument('-t', '--target_location', type=str, required=True, help='Encrypted files destination - e.g. "/mnt/loadstore/AmRes/Compliance/Bridger/Daily/encrypted"')
    ...
    parser.add_argument('-p', '--filename_contains_pattern', type=str, required=True, help='Check if files contain proper name pattern - e.g. *AmherstResidential*.txt')
...
if __name__ == '__main__':
    ...
    log = AmherstLogger(log_directory=args.log, log_file_app=py_file, vendor_name='AmRes',
                        vendor_product='DailyEncrypter')
