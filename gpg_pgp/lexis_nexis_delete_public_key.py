...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Python program for deleting GnuPGP Public Key by using a Fingerprint placed on .txt file.',
        add_help=True)
    ...
    parser.add_argument('-l', '--log', type=str, required=False, help='Specify path to the log directory', default='/etl/log/')
...
if __name__ == '__main__':
    ...
    log = AmherstLogger(log_directory=args.log, log_file_app=py_file, vendor_name='LexisNexis',
                        vendor_product='DeletePublicKey')
    ...
    # getting public key fingerprint from file (with .txt extension)
    log.info('Starting Deletion process...')

    if key.delete_public_key(file_location=args.file_location, file_name=args.file_name):
        ...
        log.error('An error has occurred while deleting Public Key...')
